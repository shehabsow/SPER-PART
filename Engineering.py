...
def display_tab(tab_name):
    ...
    st.markdown(f"""
        ...
        <label class="custom-label-{tab_name}">Select row number : {tab_name}</label>
    ...
    st.markdown(f"""
        ...
        <div style='font-size: 17px; color: green;'>Current Quantity: {df_f.loc[row_number, 'Qty.']}</div>
        """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
        ...
        <label class="custom-quantity-{tab_name}">Enter quantity : {tab_name} </label>
        """, unsafe_allow_html=True)
